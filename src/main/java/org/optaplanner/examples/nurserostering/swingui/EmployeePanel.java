...
public class EmployeePanel extends JPanel {
	...
	public EmployeePanel(NurseRosteringPanel nurseRosteringPanel,
			List<ShiftDate> shiftDateList, List<Shift> shiftList,
			Employee employee) {
	...
	private void createUI() {
		...
		if (employee != null) {
			// labelAndDeletePanel.add(new
			// JLabel(nurseRosteringPanel.getEmployeeIcon()),
			// BorderLayout.WEST);
		}
		...
		if (employee != null) {
			...
			deleteButton = SwingUtils.makeSmallButton(
					new JButton(nurseRosteringPanel.getDeleteEmployeeIcon()));
			deleteButton.setToolTipText("Delete");
			deleteButton.addActionListener(
					e -> nurseRosteringPanel.deleteEmployee(employee));
		...
		resetShiftListPanel();
		numberOfShiftAssignmentsLabel = new JLabel("0 assignments",
				JLabel.RIGHT);
		numberOfShiftAssignmentsLabel
				.setPreferredSize(new Dimension(EAST_HEADER_WIDTH, 20));
	...
	public void resetShiftListPanel() {
		...
		WeekendDefinition weekendDefinition = (employee == null)
		...
		for (ShiftDate shiftDate : shiftDateList) {
			...
			Color backgroundColor = weekendDefinition
					.isWeekend(shiftDate.getDayOfWeek())
			...
			if (employee != null) {
				...
				} else if (employee.getDayOnRequestMap()
						.containsKey(shiftDate)) {
					backgroundColor = TangoColorFactory.SCARLET_1;
				} else if (employee.getHolidayRequestMap()
						.containsKey(shiftDate)) {
					backgroundColor = TangoColorFactory.ALUMINIUM_4;
				}
			...
			boolean inPlanningWindow = nurseRosteringPanel.getSolution()
			...
			shiftDatePanel.setBorder(BorderFactory.createCompoundBorder(
					BorderFactory.createLineBorder(inPlanningWindow
							? TangoColorFactory.ALUMINIUM_6
							: TangoColorFactory.ALUMINIUM_3),
		...
		for (Shift shift : shiftList) {
			...
			if (employee != null) {
				...
				else if (employee.getRosterdayMap().containsKey(shift)) {
					backgroundColor = TangoColorFactory.SCARLET_1;
				}
				else if (employee.getLeaveMap().containsKey(shift)) {
					backgroundColor = TangoColorFactory.SCARLET_1;
				}
			}
			shiftPanel.setBackground(backgroundColor);
			shiftPanel.setToolTipText("<html>Date: "
					+ shift.getShiftDate().getLabel() + "<br/>" + "Employee: "
	...
	public void addShiftAssignment(ShiftAssignment shiftAssignment) {
		...
		JButton shiftAssignmentButton = SwingUtils.makeSmallButton(
				new JButton(new ShiftAssignmentAction(shiftAssignment)));
		...
		if (employee != null) {
			if (employee.getDayOffRequestMap().containsKey(shift.getShiftDate())
				...
				shiftAssignmentButton
						.setForeground(TangoColorFactory.SCARLET_1);
			}
		}
	...
	public void update() {
		numberOfShiftAssignmentsLabel
				.setText(shiftAssignmentButtonMap.size() + " assignments");
	}
	...
	private class ShiftAssignmentAction extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			List<Employee> employeeList = nurseRosteringPanel.getSolution()
					.getEmployeeList();
			// Add 1 to array size to add null, which makes the entity
			// unassigned
			...
			int result = JOptionPane.showConfirmDialog(
					EmployeePanel.this.getRootPane(), employeeListField,
					"Select employee", JOptionPane.OK_CANCEL_OPTION);
			if (result == JOptionPane.OK_OPTION) {
				Employee toEmployee = (Employee) employeeListField
						.getSelectedItem();
				nurseRosteringPanel.moveShiftAssignmentToEmployee(
						shiftAssignment, toEmployee);
			}
		}
