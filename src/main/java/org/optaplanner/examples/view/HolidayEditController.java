...
public class HolidayEditController implements Initializable {
	...
	private ObservableList<HolidaysData> HolidaysDataList = FXCollections
			.observableArrayList();
	...
	public ObservableList<HolidaysData> getHolidaysDataList() {
		...
		HolidaysDataList = FXCollections.observableList(
				(List<HolidaysData>) rosterService.listHolidaysData());
	...
	public void setHoliday(HolidaysData holidaydata) {
		...
		startdate.setValue(holidaydata.getStartdate());
		enddate.setValue(holidaydata.getEnddate());
	}
	...
	if (isDateInputValid()) {
			...
			okClicked = true;
			rosterService.updateHolidaysData(holidaydata);
